...
Some migrations need to run without transaction wrapping (e.g., when changing SQLite PRAGMA settings). Set `NoTransaction: true` for these cases:

```go
Register(Migration{
    Version: "20260108_154500_example",
    Name:    "Example migration without transaction",
    NoTransaction: true, // Required for PRAGMA changes
    Up: func(tx *gorm.DB) error {
    ...
    Down: func(tx *gorm.DB) error {
        // Similar pattern for rollback
        return nil
    },
})
```

**Note:** Be extra careful with `NoTransaction: true` migrations as they can't be automatically rolled back if they fail partway through.
