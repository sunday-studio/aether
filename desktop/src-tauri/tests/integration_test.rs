...
async fn test_health_check() -> Result<()> {
    let base_url =
        std::env::var("TEST_API_URL").unwrap_or_else(|_| "http://localhost:9119".to_string());
    ...
    let response = client.get(&format!("{}/v1/ping", base_url)).send().await?;
...
async fn test_tag_crud() -> Result<()> {
    ...
    // Get all tags
    let list_response = client.get(&format!("{}/v1/tags", base_url)).send().await?;
