    ...
    pub fn new(database: Arc<Database>, app_handle: AppHandle) -> Self {
        ...
        tokio::spawn(async move {
            while let Some(job) = receiver.recv().await {
                if let Err(e) = process_transcription_job(job.clone(), db_clone.clone(), app_clone.clone()).await {
...
async fn process_transcription_job(
    ...
    repo.update_status(&job.transcription_id, "processing", None, None, None).await?;
    
    // Emit status update
    // TODO: Fix event emission for Tauri 2
    // For now, events will be handled via polling or direct status checks
    tracing::info!("Transcription processing: media_id={}, transcription_id={}", job.media_id, job.transcription_id);
    
    // Load original audio file
    let audio_data = read_audio_file(database.clone(), &job.media_id).await?;
    
    // Detect format from file
    let format = crate::audio::recorder::detect_format(&audio_data)
        .unwrap_or_else(|| "webm".to_string());
    
    // Preprocess/compress audio
    let compressed_audio = compress_audio_bytes(&audio_data, &format).await
    ...
    let provider: Box<dyn TranscriptionProvider> = match job.provider_name.as_str() {
        "openai" => {
            ...
            p.initialize().await.map_err(|e| AppError::Internal(format!("Provider initialization failed: {}", e)))?;
            Box::new(p)
        }
        "groq" => {
        ...
        _ => return Err(AppError::BadRequest(format!("Unknown provider: {}", job.provider_name))),
    };
    ...
    match result {
        Ok(transcription_result) => {
            ...
            // TODO: Fix event emission for Tauri 2
            tracing::info!("Transcription complete: media_id={}, transcription_id={}", job.media_id, job.transcription_id);
        }
        Err(e) => {
            ...
            // Emit error event
            // TODO: Fix event emission for Tauri 2
            tracing::error!("Transcription failed: media_id={}, transcription_id={}, error={}", job.media_id, job.transcription_id, e);
        }
    }
